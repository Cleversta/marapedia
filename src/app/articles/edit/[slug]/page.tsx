...
// ─── Same editor-type logic as create page ────────────────────────────────────
...
export default function EditArticlePage() {
  ...
  async function handleSave() {
    ...
    // Save revision for current lang
    ...
    // Upsert translations
    const filled = (Object.keys(langs) as Language[]).filter(
      l => langs[l].title.trim() && langs[l].content.trim()
    )
  ...
  // ─── Reacts to category, switches editor automatically ────────────────────
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-bold">
            {currentCat?.icon} Edit {currentCat?.label} Article
          </h1>
        ...
        <div className="flex gap-2 items-center">
          {success && (
            <span className="text-sm text-green-700 bg-green-50 px-3 py-1.5 rounded-lg">{success}</span>
          )}
          <button
            onClick={handleSave}
            disabled={saving}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className={`w-full border border-gray-300 rounded-lg focus:outline-none focus:border-green-600 transition-colors ${
                editorType === 'poem'
                  ? 'px-3 py-2.5 text-xl font-display text-center'
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* Category — switching this changes the editor */}
          ...
          <div className={`rounded-xl p-4 text-xs leading-relaxed border ${
            ...
            {editorType === 'poem' && (
              <><strong>Editing a poem.</strong> Each line is a verse line. Blank line = new stanza.</>
            )}
            {editorType === 'song' && (
              <><strong>Editing song lyrics.</strong> Use + to add Verse, Chorus, Bridge sections.</>
            )}
            {editorType === 'rich' && (
              <><strong>Tip:</strong> Switch language tabs to edit other language versions.</>
            )}
          </div>
        </div>
      </div>
