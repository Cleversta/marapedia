...
export default function Navbar() {
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') { e.preventDefault(); setMobileMenuOpen(false); searchRef.current?.focus() }
      if (e.key === 'Escape') { setMenuOpen(false); setMobileMenuOpen(false); searchRef.current?.blur() }
    }
  ...
  // Categories with custom href (e.g. Photos → /photos) use that href,
  // otherwise fall back to /category/:value
  ...
  function UserAvatar({ size = 8 }: { size?: number }) {
    ...
    if (profile?.avatar_url)
      return <img src={profile.avatar_url} alt={profile.username} style={{ width: dim, height: dim }}
  ...
  function RoleBadge({ className = '' }: { className?: string }) {
    ...
    if (profile.role === 'admin') return <span className={`inline-flex items-center text-[10px] px-1.5 py-0.5 rounded font-medium bg-purple-100 text-purple-700 ${className}`}>Admin</span>
    if (profile.role === 'editor') return <span className={`inline-flex items-center text-[10px] px-1.5 py-0.5 rounded font-medium bg-blue-100 text-blue-700 ${className}`}>Editor</span>
    return null
  }
    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4">
            ...
            {user ? (
              <>
                ...
                <div className="relative" ref={menuRef}>
                  <button onClick={() => setMenuOpen(!menuOpen)} aria-expanded={menuOpen} aria-haspopup="true"
                    className="flex items-center gap-2 text-sm pl-1 pr-2 py-1 rounded-lg hover:bg-gray-100 active:bg-gray-200 transition-colors duration-150">
                    <UserAvatar size={8} />
                    <span className="hidden md:block text-sm font-medium text-gray-800">{profile?.username}</span>
                    <RoleBadge className="hidden md:inline-flex" />
                    <svg className={`hidden md:block w-3.5 h-3.5 text-gray-400 transition-transform duration-200 ${menuOpen ? 'rotate-180' : ''}`}
                  ...
                  {menuOpen && (
                    <div className="absolute right-0 mt-1.5 w-56 bg-white border border-gray-200
                      ...
                      <div className="py-1">
                        ...
                        <Link href="/articles/my" onClick={() => setMenuOpen(false)}
                          className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                          <span className="text-base">📑</span> My Articles
                        </Link>
                        {isEditor && (
                          <>
              ...
              <>
                <Link href="/login" className="text-sm px-3 py-1.5 border border-gray-300 rounded-lg hover:bg-gray-50 active:scale-95 transition-all duration-150">Login</Link>
                <Link href="/register" className="text-sm px-3 py-1.5 bg-green-700 text-white rounded-lg hover:bg-green-800 active:scale-95 transition-all duration-150 font-medium">Register</Link>
              </>
            )}
        ...
        {/* ── Category nav — desktop ─────────────────────────────────────────
            Photos tab is last, links to /photos instead of /category/photos  */}
        <nav className="hidden md:flex items-center overflow-x-auto scrollbar-hide border-t border-gray-100"
          ...
          {CATEGORIES.map((cat, i) => (
            <Link key={cat.value} href={catHref(cat)}
              ...
              <span>{cat.label}</span>
              {i === 0 && (
                <span className="ml-1 text-[9px] text-gray-400 border border-gray-200 rounded px-1 py-0.5 leading-none select-none">Tab</span>
              )}
            </Link>
          ))}
        </nav>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-4 space-y-4">
            ...
            <div className="relative">
              <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                ...
                className="w-full pl-4 pr-10 py-2 text-sm border border-gray-300 rounded-full
                  bg-gray-50 focus:bg-white focus:outline-none focus:border-green-600 focus:ring-2 focus:ring-green-600/20 transition-all" />
              <button type="submit" aria-label="Search"
          ...
          {/* All categories including 📷 Photos */}
          <div>
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2 px-1">Browse</p>
            <div className="grid grid-cols-2 gap-1">
              {CATEGORIES.map(cat => (
                <Link key={cat.value} href={catHref(cat)} onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-2 text-sm px-3 py-2 rounded-lg transition-colors ${
                    isCatActive(cat) ? 'bg-green-50 text-green-800 font-medium' : 'text-gray-600 hover:bg-gray-50'
                  }`}>
          ...
          {user ? (
            <div className="space-y-2 pt-1">
              <Link href="/articles/create" onClick={() => setMobileMenuOpen(false)}
                className="flex items-center justify-center gap-2 text-sm px-3 py-2.5
                  bg-green-700 text-white rounded-xl hover:bg-green-800 active:scale-95 transition-all duration-150 font-medium">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" />
                </svg>
                Contribute Article
              ...
              <button onClick={handleLogout}
                className="w-full flex items-center justify-center gap-2 text-sm px-3 py-2.5
                  border border-red-200 text-red-600 rounded-xl hover:bg-red-50 active:scale-95 transition-all duration-150">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
            ...
            <div className="flex gap-2 pt-1">
              <Link href="/login" onClick={() => setMobileMenuOpen(false)}
                className="flex-1 text-center text-sm px-3 py-2.5 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">Login</Link>
              <Link href="/register" onClick={() => setMobileMenuOpen(false)}
                className="flex-1 text-center text-sm px-3 py-2.5 bg-green-700 text-white rounded-xl hover:bg-green-800 transition-colors font-medium">Register</Link>
            </div>
          )}
        </div>
      )}
